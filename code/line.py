...
def get_line():
    ...
    parser.add_argument('--graph_file', default='facebook_graph.pkl')
    ...
    with tf.Session() as sess:
        ...
        for b in range(args.num_batches):
            ...
            if b % 1000 == 0 or b == (args.num_batches - 1):
                ...
                context_embedding =sess.run(model.context_embedding)
                normalized_context_embedding=context_embedding/np.linalg.norm(context_embedding,axis=1,keepdims=True)
                # pickle.dump(data_loader.embedding_mapping(normalized_embedding),open('data/embedding_%s.pkl' % suffix, 'wb'))
                # pickle.dump(data_loader.embedding_mapping(normalized_context_embedding),open('data/context_embedding_%s.pkl' % suffix,'wb'))
    return  data_loader.embedding_mapping(normalized_embedding), data_loader.embedding_mapping(normalized_context_embedding)
